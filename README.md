...
- **Privacy-conscious AI prompt tools:** From a prayer request or saved diary reflection, **Find Scripture with AI** prepares a prompt asking for relevant Bible passages and YouVersion links, while **Listen to a Message with AI** prepares a prompt for a short pastoral message that can be read or played with the chosen assistant's read-aloud features when available. Gathered does not automatically send private content to an AI provider; the user chooses whether to copy or share a generated prompt and which AI assistant receives it.
...
Photos and videos are stored locally in a dedicated IndexedDB store rather than in application-state JSON. New diary photos are processed entirely on the device: Gathered corrects their displayed orientation, resizes them to fit within 256 × 256 pixels without upscaling, compresses them, and removes unnecessary camera/EXIF metadata before encrypted storage. Original full-resolution photos are not retained, so Diary is not a full-resolution photo archive. Video behavior is unchanged.

Each attachment is authenticated and encrypted with the same unlocked AES-GCM data key as the diary text, and only encrypted attachment records are included in version 2 encrypted backups. Gathered never uploads diary content. Native browser storage quotas still apply—especially for large videos—and attachment failures leave an existing entry intact.
