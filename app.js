...
function defaultState() {
  return { version:4, group:null, members:[], entries:[], prayers:[], followUps:[], settings:{ translation:'NIV', youVersionApiKey:'', lastBackupAt:null } };
}
...
function openDB() {
  return new Promise((resolve,reject)=>{
    ...
    req.onupgradeneeded=()=>{ const db=req.result; if(!db.objectStoreNames.contains(STATE_STORE)) db.createObjectStore(STATE_STORE); };
...
function homePage(){
  ...
  return shell(`<div class="page"><section class="hero"><div class="eyebrow">Gathered</div><h1>${esc(state.group.name)}</h1><p class="subtle">Capture the Word, remember your people, and carry prayer forward from week to week.</p></section>${backupDue()?`<div class="notice backup-notice"><strong>Backup recommended.</strong> It has been ${state.settings.lastBackupAt?daysSince(state.settings.lastBackupAt)+' days':'more than 30 days'} since your last recorded export. <a href="#settings">Back up now</a>.</div>`:''}<a class="fab-card" href="#entry/new"><div><h2>New Session</h2><p>Scripture · journal · prayer · follow-ups</p></div><div class="icon">＋</div></a><section class="section"><div class="grid three"><div class="card stat"><strong>${state.members.length}</strong><span>Members</span></div><div class="card stat"><strong>${active.length}</strong><span>Active prayers</span></div><div class="card stat"><strong>${openFollowUps.length}</strong><span>Open follow-ups</span></div></div></section><section class="section"><div class="section-title"><h2>Prayer focus</h2><a class="btn small secondary" href="#prayers">View all</a></div>${active.length?`<div class="list">${active.slice(0,3).map(prayerListItem).join('')}</div>`:`<div class="empty">No active prayer requests yet.</div>`}</section><section class="section"><div class="section-title"><h2>Recent sessions</h2><a class="btn small secondary" href="#entries">View all</a></div>${recent.length?`<div class="list">${recent.map(entryListItem).join('')}</div>`:`<div class="empty">Your first Gathered session will appear here.</div>`}</section></div>`,'home');
}
...
function prayerListItem(p){return `<a class="list-item clickable" href="#prayer/${p.id}" style="text-decoration:none"><div class="list-main"><strong>${esc(p.memberName||getMemberName(p.memberId,'General / group'))}</strong><div class="meta">${esc(p.text)}</div></div><span class="pill ${p.status==='answered'?'answered':''}">${p.status==='answered'?'Answered':'Active'}</span></a>`;}
...
function prayerDetail(id){const p=getPrayer(id);if(!p)return prayersPage();const history=[{date:p.createdDate,text:'Prayer request created',entryId:p.createdEntryId},...(p.updates||[]).map(u=>({date:u.date,text:u.text,entryId:u.entryId}))].sort((a,b)=>(b.date||'').localeCompare(a.date||''));return shell(`<div class="page"><div class="section-title"><div><div class="eyebrow">Prayer history</div><h1>${esc(p.memberName||getMemberName(p.memberId,'General / group'))}</h1></div><span class="pill ${p.status==='answered'?'answered':''}">${p.status==='answered'?'Answered':'Active'}</span></div><div class="card flat"><div class="detail-body"><strong>${esc(p.text)}</strong></div><div class="meta">Started ${fmtDate(p.createdDate)}${p.answeredDate?` · Answered ${fmtDate(p.answeredDate)}`:''}</div></div><section class="section"><div class="section-title"><h2>Timeline</h2></div><div class="timeline">${history.map(h=>`<div class="timeline-item"><div class="timeline-dot"></div><div><strong>${fmtDate(h.date)}</strong><div class="detail-body">${esc(h.text)}</div>${h.entryId?`<a class="mini-link" href="#entry/${h.entryId}">Open session</a>`:''}</div></div>`).join('')}</div></section></div>`,'prayers');}

function followUpItem(f){return `<div class="list-item"><div class="list-main"><strong>${esc(f.text)}</strong><div class="meta">${esc(f.memberName||getMemberName(f.memberId,'Unassigned'))}${f.dueDate?` · Due ${fmtDate(f.dueDate)}`:''}</div></div><span class="pill ${f.status==='completed'?'answered':''}">${f.status==='completed'?'Done':'Open'}</span></div>`;}
...
function bindSearch(){const input=document.getElementById('globalSearch'),results=document.getElementById('searchResults');const renderResults=()=>{const q=input.value.trim().toLowerCase();if(!q){results.innerHTML='<div class="empty">Start typing to search your small group history.</div>';return;}const hits=[];state.members.forEach(m=>{const hay=[m.name,m.role,m.email,m.phone,m.notes].join(' ').toLowerCase();if(hay.includes(q))hits.push({type:'Member',title:m.name,meta:m.role||'Member profile',href:`#member/${m.id}`});});state.entries.forEach(e=>{const hay=[e.scripture,e.journal].join(' ').toLowerCase();if(hay.includes(q))hits.push({type:'Session',title:e.scripture||'Session',meta:fmtDate(e.date),href:`#entry/${e.id}`});});state.prayers.forEach(p=>{const hay=[p.text,p.memberName,...(p.updates||[]).map(u=>u.text)].join(' ').toLowerCase();if(hay.includes(q))hits.push({type:'Prayer',title:p.memberName||'Prayer',meta:p.text,href:`#prayer/${p.id}`});});state.followUps.forEach(f=>{const hay=[f.text,f.memberName].join(' ').toLowerCase();if(hay.includes(q))hits.push({type:'Follow-up',title:f.text,meta:f.memberName||'Unassigned',href:`#entry/${f.createdEntryId}`});});results.innerHTML=hits.length?`<div class="list">${hits.slice(0,50).map(h=>`<a class="list-item clickable" href="${h.href}" style="text-decoration:none"><div class="list-main"><span class="kicker">${esc(h.type)}</span><strong>${esc(h.title)}</strong><div class="meta">${esc(h.meta)}</div></div><span>›</span></a>`).join('')}</div>`:'<div class="empty">No matches.</div>';};input.addEventListener('input',renderResults);renderResults();input.focus();}
