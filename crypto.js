...
const sessionLabel = value => value === 'sunday-worship' ? 'Sunday Worship' : 'Small Group';
...
document.addEventListener('click',async e=>{const choice=e.target.closest('[data-session-choice]');if(choice){choice.disabled=true;await createDraft(choice.dataset.sessionChoice);return;}const del=e.target.closest('[data-delete-draft]');if(del&&confirm('Delete this draft session? This cannot be undone.')){state.entries=state.entries.filter(x=>x.id!==del.dataset.deleteDraft);await saveState();render();return;}const deletePrayer=e.target.closest('[data-delete-prayer-request]');if(deletePrayer&&confirm('Delete this prayer request? This will permanently delete the request and all of its updates. This cannot be undone.')){const id=deletePrayer.dataset.deletePrayerRequest;state.prayers=state.prayers.filter(p=>p.id!==id);state.entries.forEach(s=>{s.prayerIds=(s.prayerIds||[]).filter(x=>x!==id);if(s.draftData)s.draftData.prayerUpdates=(s.draftData.prayerUpdates||[]).filter(x=>x.prayerId!==id);});await saveState();location.hash='#prayers';render();}});
...
bindSettings=function(){oldBindSettings();document.getElementById('lockGathered').onclick=async()=>{clearTimeout(draftTimer);await saveChain;state=defaultState();cryptoSession.dek=null;cryptoSession.mode='locked';location.hash='';render();};document.getElementById('changePassphrase').onclick=()=>changePassphraseDialog();document.getElementById('resetApp').onclick=async()=>{if(confirm('Reset Gathered? An encrypted safety backup will download first, then all local Gathered data will be deleted.')){await downloadBackup('pre-reset',false);await dbDelete(ENVELOPE_KEY);await dbDelete(STATE_KEY);localStorage.removeItem(LEGACY_STORAGE_KEY);state=defaultState();cryptoSession={dek:null,envelope:null,mode:'setup',legacy:null};location.hash='';render();}};const input=document.getElementById('importData');input.onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(data.backupFormat==='gathered-encrypted-backup'&&data.envelope){const pass=prompt("Enter this backup's passphrase");if(pass===null)return;const restored=await unlockEnvelope(data.envelope,pass);if(!confirm(`Restore encrypted backup for “${restored.state.group?.name||'Gathered'}”? Your current encrypted data will be exported first.`))return;await downloadBackup('pre-import',false);state=restored.state;await saveState();location.hash='#home';render();toast('Encrypted backup restored');return;}const check=validateBackup(data);if(!check.valid)throw new Error(check.errors.join('\n'));if(!confirm('This is an older unencrypted backup. Import it and immediately protect it with your current Gathered encryption?'))return;await downloadBackup('pre-import',false);state=migrateState(data);await saveState();location.hash='#home';render();toast('Legacy backup imported and encrypted');}catch{alert('The backup could not be decrypted or validated. Check its passphrase and file.');}};};
function changePassphraseDialog(){const current=prompt('Current passphrase');if(current===null)return;const next=prompt('New passphrase (at least 12 characters; use a long memorable phrase)');if(next===null)return;if(!estimatePassphrase(next).acceptable){alert('Choose a stronger passphrase.');return;}const confirmNext=prompt('Confirm new passphrase');if(next!==confirmNext){alert('Passphrases do not match.');return;}unlockEnvelope(cryptoSession.envelope,current).then(async({dek})=>{const salt=randomBytes(16),kek=await deriveKek(next,salt),wrapped=await crypto.subtle.wrapKey('raw',dek,kek,'AES-KW');const candidate={...cryptoSession.envelope,kdf:{name:'PBKDF2',hash:'SHA-256',iterations:KDF_ITERATIONS,salt:b64(salt)},wrappedDek:b64(wrapped)};await dbPut(ENVELOPE_KEY,candidate);await unlockEnvelope(await dbGet(ENVELOPE_KEY),next);cryptoSession.envelope=candidate;toast('Passphrase changed');}).catch(()=>alert('Incorrect current passphrase. No changes were made.'));}
