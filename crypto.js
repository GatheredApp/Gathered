...
async function makeEnvelope(snapshot,passphrase) {
  ...
  return {dek,envelope:{format:'gathered-encrypted-state',formatVersion:CRYPTO_FORMAT,schemaVersion:5,kdf:{name:'PBKDF2',hash:'SHA-256',iterations:KDF_ITERATIONS,salt:b64(salt)},wrappedDek:b64(wrapped),state:encrypted,updatedAt:new Date().toISOString()}};
}
...
saveState = function () {
  ...
  saveChain=saveChain.catch(()=>{}).then(async()=>{
    ...
    const envelope={...cryptoSession.envelope,schemaVersion:5,state:encrypted,revision,updatedAt:new Date().toISOString()};
